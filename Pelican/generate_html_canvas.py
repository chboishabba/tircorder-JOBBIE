...
def load_facebook_messages(messages_dir, items):
    for json_file in Path(messages_dir).rglob('*.json'):
        ...
        for msg in data.get('messages', []):
            ...
            items.append({'ts': ts / 1000, 'text': text})
...
def build_points(items):
    ...
    for it in items:
        ...
        points.append({'x': x, 'y': y, 'z': z, 'size': impact / 2, 'color': color})
    return points
...
if __name__ == '__main__':
    ...
    parser.add_argument('--output', default='content/cloud.html', help='Output HTML file')
